const BIRD_SVG_PATH = `<path d="M560 128C573.2 128 580.7 143.1 572.8 153.6L544 192L544 368C544 447.5 479.5 512 400 512L288 512L241.7 558.3C231.3 568.7 215.2 570.7 202.6 563.1L105.5 504.9C88.5 494.7 90.5 469.4 108.9 462L224 416C87.8 375.1 71.5 233.8 86 159.7C89.6 141.9 109.3 135.4 125.3 144.2L384 288L384 208C384 163.8 419.8 128 464 128L560 128zM464 184C450.7 184 440 194.7 440 208C440 221.3 450.7 232 464 232C477.3 232 488 221.3 488 208C488 194.7 477.3 184 464 184zM246.5 54.4C258.9 40.7 279.8 45.5 289 61.5L345.4 159.8C339.6 174.2 336.2 189.9 336 206.3L202.1 132C212.2 100.4 229.1 73.6 246.5 54.4z"/>`;

const NUMBER_OF_BIRDS = 5;
let birdsInitialized = false;

const FLY_ANIMATIONS = [
    'fly-path-1',
    'fly-path-2',
    'fly-path-3',
    'fly-path-4'
];


export function initializeFlyingBirds() {
    if (birdsInitialized) return;

    const container = document.getElementById('flying-birds-container');
    if (!container) {
        console.error('Container burung tidak ditemukan di Hero Section.');
        return;
    }
    
    container.classList.remove('hidden');
    console.log("Burung mulai terbang di Hero Section dengan jalur acak!");

    for (let i = 0; i < NUMBER_OF_BIRDS; i++) {
        const bird = document.createElement('div');
        
        const randomPath = FLY_ANIMATIONS[Math.floor(Math.random() * FLY_ANIMATIONS.length)];
        
        const size = Math.floor(Math.random() * (40 - 20) + 20);
        const delay = Math.random() * 10;
        const duration = Math.random() * (30 - 15) + 15;
        
        bird.className = `bird absolute text-white opacity-70`; 
        
        bird.style.width = `${size}px`;
        bird.style.height = `${size}px`;
        bird.style.animationDelay = `${delay}s`;
        bird.style.animationDuration = `${duration}s`;
        
        bird.style.animationName = randomPath; 
        
        bird.innerHTML = `
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" fill="currentColor">
                ${BIRD_SVG_PATH}
            </svg>
        `;
        
        container.appendChild(bird);
    }
    
    birdsInitialized = true;
}